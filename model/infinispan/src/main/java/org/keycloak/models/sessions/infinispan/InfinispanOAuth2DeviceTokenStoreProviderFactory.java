        ...
        if (codeCache == null) {
            synchronized (this) {
                if (codeCache == null) {
                    ...
                    if (remoteCache != null) {
                        LOG.debugf("Having remote stores. Using remote cache '%s' for token of oauth2 device grant", remoteCache.getName());
                        this.codeCache = () -> {
                            // Doing this way as flag is per invocation
                            return remoteCache.withFlags(Flag.FORCE_RETURN_VALUE);
                        };
                    } else {
                        LOG.debugf("Not having remote stores. Using normal cache '%s' for token of oauth2 device grant", cache.getName());
                        this.codeCache = () -> {
                            return cache;
                        };
                    }
