...
public class OIDCLoginProtocol implements LoginProtocol {
    ...
    public Response authenticated(AuthenticationSessionModel authSession, UserSessionModel userSession, ClientSessionContext clientSessionCtx) {
        ...
        // OAuth2 device grant
        String verifiedUserCode = authSession.getAuthNote(OIDCLoginProtocol.OAUTH2_DEVICE_VERIFIED_USER_CODE);
        if (verifiedUserCode != null) {
            String userSessionId = clientSession.getUserSession().getId();
            OAuth2DeviceTokenStoreProvider store = session.getProvider(OAuth2DeviceTokenStoreProvider.class);
            if (store.approve(realm, clientSession.getClient(), verifiedUserCode, userSessionId)) {
                URI redirectUrl = OIDCLoginProtocolService.delegatedUrl(uriInfo).build(realm.getId());
                return Response.status(302).location(redirectUrl).build();
            } else {
                // Expired
                throw new ErrorPageException(session, authSession, Response.Status.BAD_REQUEST, Messages.EXPIRED_USER_CODE);
            }
        }
    ...
    public Response sendError(AuthenticationSessionModel authSession, Error error) {
        if (isOAuth2DeviceVerificationRequired(authSession)) {
            ...
            return session.getProvider(LoginFormsProvider.class)
                    ...
                    .createErrorPage(Response.Status.OK);
        }
