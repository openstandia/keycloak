...
public class TokenEndpoint {
    ...
    public Response deviceCodeToToken() {
        if (!client.isOAuth2DeviceGrantEnabled()) {
            event.error(Errors.NOT_ALLOWED);
            throw new CorsErrorResponseException(cors, OAuthErrorException.INVALID_GRANT, "Client not allowed device grant", Response.Status.BAD_REQUEST);
        }
        ...
        OAuth2DeviceCodeModel deviceCodeModel = store.get(realm, client, deviceCode);
        ...
        if (!store.checkPollingInterval(deviceCodeModel, realm.getOAuth2DevicePollingInterval())) {
        ...
        store.remove(deviceCodeModel);

        // Retrieve UserSession
        UserSessionModel userSession = new UserSessionCrossDCManager(session)
                .getUserSessionWithClient(realm, deviceCodeModel.getUserSessionId(), client.getId());

        if (userSession == null) {
            userSession = session.sessions().getUserSession(realm, deviceCodeModel.getUserSessionId());
            if (userSession == null) {
                event.error(Errors.USER_SESSION_NOT_FOUND);
                throw new CorsErrorResponseException(cors, OAuthErrorException.INVALID_GRANT, "User session not found", Response.Status.BAD_REQUEST);
            }
